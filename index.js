    ...
    try {
        for (const entry of body.entry || []) {
            for (const messagingEvent of entry.messaging || []) {
                ...
                if (!rawText) {
                    console.log('[Webhook] Evento sin mensaje de texto.');
                    continue;
                }
...
async function sendInstagramMessage(recipientId, messageText) {
    try {
        ...
        console.error('[BOT ERROR] Fallo al enviar respuesta en Instagram:', JSON.stringify(error.response?.data || error.message, null, 2));
    }
}

app.listen(PORT, () => {
    console.log(`Servidor de Instagram escuchando en el puerto ${PORT}`);
    console.log('[BOT] Versión webhook diagnóstico 1');
});
